...
KEYWORD_CONFIG = {
    ...
    "desk": {
        ...
        # 데스크 셋업과 무관한 뉴스 제외
        "exclude_patterns": [
            r"(드라마|영화|예능|방송|출연|촬영|시청률)",  # 연예/방송
            r"(가수|배우|아이돌|연예인|셀럽)",  # 연예인
            r"(콘서트|공연|무대|음원|싱글|앨범)",  # 음악
            r"(넷플릭스|디즈니|왓챠|티빙|쿠팡플레이)",  # OTT
            r"\[.*포토\]|\[.*사진\]|\[.*영상\]",  # 포토/영상 뉴스
            r"(채용|구인|모집|입사|취업)",  # 채용 관련
            r"(올림픽|월드컵|경기|승리|패배|우승)",  # 스포츠
            r"(사건|사고|범죄|재판|판결|구속)",  # 사건/사고
            r"(자동차|SUV|세단|전기차|하이브리드)",  # 자동차
        ],
        # 실제 데스크 셋업 뉴스 우선
        "boost_patterns": [
            r"(모니터|키보드|마우스|마우스패드|장패드)",
            r"(스피커|사운드바|DAC|앰프)",
            r"(의자|책상|거치대|모니터암)",
            r"(리뷰|추천|비교|언박싱|개봉기)",
            r"(게이밍|오피스|사무용|업무용)",
            r"(QHD|4K|OLED|IPS|VA|Hz)",
        ],
    },
}
